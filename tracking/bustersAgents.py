...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefs in livingGhostPositionDistributions:
            thisProb, pos = findMostLikelyPos(beliefs)
            if thisProb > prob:
                goalPos = pos
                prob = thisProb
        ...
        result = legal[0]
        for action in legal:
            thisEffort = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,action),goalPos)
            if thisEffort < effort:
                effort = thisEffort
                result = action
        return result