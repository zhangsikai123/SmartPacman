...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.allPositions:
            newP = self.getObservationProb(observation,gameState.getPacmanPosition(),p,self.getJailPosition())
            self.beliefs[p] = newP * self.beliefs[p]
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.allPositions:
            ...
            for key in newPosDist.keys():
                beliefs_copy[key] += belief * newPosDist[key]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numPerPos = self.numParticles / len(positions)
        "*** YOUR CODE HERE ***"
        for p in positions:
            for i in range(numPerPos):
                self.particles.append(p)

        residual = self.numParticles % len(positions)
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            for newpos in newPosDist.keys():
                distribution[newpos] = newPosDist[newpos]
