...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        "*** YOUR CODE HERE ***"
        conditonVariables = factor.conditionedVariables()
        unconditionedVariables = factor.unconditionedVariables()
        unconditionedVariables.remove(eliminationVariable)
        newFactor = Factor(unconditionedVariables, conditonVariables, factor.variableDomainsDict())

        for assignment in newFactor.getAllPossibleAssignmentDicts():
            prob = 0
            tempAssignment = assignment
            for eliminVal in factor.variableDomainsDict()[eliminationVariable]:
                tempAssignment[eliminationVariable] = eliminVal
                prob += factor.getProbability(tempAssignment)
            newFactor.setProbability(tempAssignment,prob)
...
def normalize(factor):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        if sum==0:
            #random action
            newFactor.setProbability(assignment, 0.1)
        else:
