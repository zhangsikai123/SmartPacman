...
def constructBayesNet(gameState):
    ...
    obsVars = []
    variables = [X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
    "*** YOUR CODE HERE ***"
    ...
    # Populate observations: obs1, obs2 ....
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        factor = inference.inferenceByVariableElimination(self.bayesNet, queryVariables, evidence, eliminationOrder)
        assignmentDict = {}
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, exploreEvidence in self.getExplorationProbsAndOutcomes(evidence):
            totalEvidence = evidence
            totalEvidence.update(exploreEvidence)
            expectedValue += prob  * max (self.computeEnterValues(totalEvidence,enterEliminationOrder))
